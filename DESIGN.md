...
| 风险 | 缓解 |
|---|---|
| 富文本 ⇄ Markdown 双向转换的保真度是最大技术难点 | ✅ **已 spike 验证**:转换层落在 `packages/editor/convert.ts`(正向用 remark 解析、反向手写序列化器),14 个往返幂等测试覆盖标题/混合标记/嵌套列表/表格/代码块/引用/图片/wikilink 全部通过。节点白名单外的内容降级为纯文本段落,不丢失 |
